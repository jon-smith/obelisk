...
void outputDebugString(const std::wstring &str) { OutputDebugStringW(std::wstring(str + L"\n").c_str()); }

void outputDebugString(const std::string &str) { OutputDebugStringA(std::string(str + "\n").c_str()); }
...
std::wstring getExecutableDirectory() { return std::experimental::filesystem::path(getExecutablePath()).parent_path(); }

namespace windows
{
bool beep(unsigned int frequencyHz, std::chrono::milliseconds time) { return Beep(static_cast<DWORD>(frequencyHz), static_cast<DWORD>(time.count())) == TRUE; }
...
bool delayLoadDLL(std::wstring dllname) { return ::LoadLibraryW(dllname.c_str()) != nullptr; }
} // namespace windows
} // namespace platform_utilities
} // namespace obelisk
