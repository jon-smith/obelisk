...
namespace obelisk
{
template <typename OutT, typename InContainerT, typename ConvertFnT> std::vector<OutT> convertAll(InContainerT &&in, ConvertFnT &&converter)
{
	std::vector<OutT> out;
	...
	-> std::vector<typename ContainerT::value_type>
{
	if (fromIndex >= container.size())
		throw std::invalid_argument(
			formatString("Index out of range during restrict increase/decrease (idx: %d, size: %d)", fromIndex, container.size()).c_str());
	...
	-> std::vector<typename ContainerT::value_type>
{
	...
	return restrictToIncreasingRange(fromIndex, container,
									 [&comparator](typename ContainerT::value_type lhs, typename ContainerT::value_type rhs) { return comparator(rhs, lhs); });
}
} // namespace obelisk