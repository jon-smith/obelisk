...
#include "obelisk/Box.hpp"
#include "obelisk/VectorT.hpp"
...
namespace obelisk
{
template <typename T> struct ImplicitGetPos
{
	static const Vector3f &getPos(const T &item) { return item; }
};

template <typename ItemT, size_t MaxItemsPerNode = 256, typename GetPosT = ImplicitGetPos<ItemT>> class Octree
{
  private:
  ...
  public:
	Octree(Vector3f center, Vector3f size) : center(center), halfsize(size * 0.5f) {}

	Octree(Box extents) : Octree(extents.center(), extents.dimensions()) {}

	Octree(Octree &&o) : center(o.center), halfsize(o.halfsize), nTotalItems(o.nTotalItems), items(std::move(o.items)), nodes(std::move(o.nodes)) {}

	~Octree() = default;

	bool hasChildren() const { return nodes[0] != nullptr; }
	...
	template <typename InterableT> void insertRange(InterableT &&items)
	{
		for (auto &&item : items)
			insert(item);
	}
	...
	template <typename TFunc> void forEachItemInSphere(const Vector3f &center, float radius, TFunc f) const
	{
		if (hasChildren())
		{
	...
	const ItemT *findNearestNeighbour(Vector3f pos, float searchRadius) const
	{
		...
		forEachItemInSphere(pos, searchRadius, [&](const ItemT &item) {
	...
	Vector3f minExtents() const { return center - halfsize; }

	Vector3f maxExtents() const { return center + halfsize; }

	Box boundingBox() const { return Box(minExtents(), maxExtents()); }

	size_t getTotalItemCount() const { return nTotalItems; }

	template <typename TFunc> void forEachItem(TFunc f) const
	{
		if (hasChildren())
		{
			for (const auto &node : nodes)
	...
	template <typename TFunc> void forEachItemInBox(const Box &box, TFunc f) const
	{
		if (hasChildren())
		{
			for (const auto &node : nodes)
