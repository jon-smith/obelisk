...
struct Box
{
	...
	constexpr Box(Vector3f min, Vector3f max) : min(min), max(max) {}

	Vector3f dimensions() const { return max - min; }
	...
	{
		return max.x >= other.min.x && other.max.x >= min.x && max.y >= other.min.y && other.max.y >= min.y && max.z >= other.min.z && other.max.z >= min.z;
	}
	...
	bool intersectsSphere(const Vector3f &center, float radius) const { return distanceToSphere(center, radius) <= 0.0f; }

	float distanceToSphere(const Vector3f &center, float radius) const { return (nearestPointWithin(center) - center).length() - radius; }
};
} // namespace obelisk