...
namespace obelisk
{
template <typename T> class SimpleConcurrentQueue
{
  public:
...
class MultipleProducerSingleConsumerQueue : private SimpleConcurrentQueue<T>
{
  public:
	...
	template <typename IteratorT> void push(IteratorT itFirst, IteratorT itLast)
	{
		UniqueLock<Mutex> lock(mutex);
	...
	template <typename RT> RT safeAccessQueue(std::function<RT(const std::deque<T> &)> fn) const
	{
