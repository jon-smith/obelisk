...
namespace obelisk
{
template <typename Container> class BackgroundContainerProducer
{
  public:
	...
	{
		containerToGet = std::async(std::launch::async, [=] { return Container(size); });
	};

	BackgroundContainerProducer(BackgroundContainerProducer &&other) : lastSize(std::move(other.lastSize)), containerToGet(std::move(other.containerToGet)) {}
	...
	Container getContainer(size_t size)
	{
		...
		lastSize = size;
		containerToGet = std::async(std::launch::async, [size] { return Container(size); });
