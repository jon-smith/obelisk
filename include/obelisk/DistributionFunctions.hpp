...
template <typename T> T gaussianPDF(T mu, T sigma, T x)
{
	return (T(1.0) / (sqrt(T(2.0) * sigma * sigma * T(pi))) * exp(-((x - mu) * (x - mu)) / (T(2.0) * (sigma * sigma))));
}

template <typename T> T gaussianCDF(T mu, T sigma, T x) { return T(0.5) * (T(1.0) + erf((x - mu) / (sigma * sqrt(T(2.0))))); }

template <typename T> T gaussianBivariatePDF(T muX, T sigmaX, T muY, T sigmaY, T x, T y)
{
...
template <typename T> T simpsons2D(std::function<T(T, T)> f, T x0, T y0, T x1, T y1, size_t halfPoints)
{
	auto const h = (x1 - x0) / static_cast<T>(2 * halfPoints);
	auto const k = (y1 - y0) / static_cast<T>(2 * halfPoints);

	auto const subX = generateVector<T>([x0, h](size_t i) { return x0 + static_cast<T>(i) * h; }, 0, halfPoints * 2);

	auto const subY = generateVector<T>([y0, k](size_t i) { return y0 + static_cast<T>(i) * k; }, 0, halfPoints * 2);
...
template <typename T> T gaussianBivariateIntegrate(T muX, T sigmaX, T muY, T sigmaY, T x0, T y0, T x1, T y1, unsigned halfPoints)
{
	auto const fn = [=](T x, T y) { return gaussianBivariatePDF<T>(muX, sigmaX, muY, sigmaY, x, y); };
