...
class EventObserver
{
	...
	template <typename EventType> void staticCheckEventType()
	{
  ...
  public:
	...
	virtual ~EventObserver() { unregisterFromAllEvents(); }

	template <typename EventType, typename ClassT, typename... Args>
	typename std::enable_if<std::is_base_of<EventObserver, ClassT>::value, void>::type registerToEvent(EventType &e, void (ClassT::*fn)(Args... args) const)
	{
	...
	template <typename EventType, typename ClassT, typename... Args>
	typename std::enable_if<std::is_base_of<EventObserver, ClassT>::value, void>::type registerToEvent(EventType &e, void (ClassT::*fn)(Args... args))
	{
		staticCheckEventType<EventType>();
	...
	template <typename EventType, typename Lambda> void registerToEvent(EventType &e, Lambda &&fn)
	{
		staticCheckEventType<EventType>();
	...
	template <typename EventType> void unregisterFromEvent(EventType &e)
	{
		staticCheckEventType<EventType>();
	...
	template <typename EventType> void eraseRegisteredEvents(EventType &e)
	{
		staticCheckEventType<EventType>();
...
template <typename EventType, typename Lambda> class ScopedEventObserver : public EventObserver
{
  public:
	ScopedEventObserver(EventType &e, Lambda &&l) { registerToEvent(e, l); }
...
template <typename EventType, typename Lambda> auto createLambdaObserver(EventType &e, Lambda &&f) -> ScopedEventObserver<EventType, Lambda>
{
