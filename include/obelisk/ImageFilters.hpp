...
#include "obelisk/DistributionFunctions.hpp"
#include <vector>
...
namespace Impl
{
template <typename T> struct FloatIfFloatOtherwiseDouble
{
	using Type = double;
};

template <> struct FloatIfFloatOtherwiseDouble<float>
{
	using Type = float;
};
} // namespace Impl
...
template <typename T> void boxBlur3x3(std::vector<T> &inputOutput, size_t width)
{
	...
	for (size_t r = 0; r < height; ++r)
	{
		...
		tmp[firstColumnIndex] = static_cast<T>(half * static_cast<CastType>(inputOutput[firstColumnIndex] + inputOutput[firstColumnIndex + 1]));

		auto const lastColumnIndex = r * width + height - 1;
		tmp[lastColumnIndex] = static_cast<T>(half * static_cast<CastType>(inputOutput[lastColumnIndex] + inputOutput[lastColumnIndex - 1]));
	}
	...
	for (size_t r = 0; r < height; ++r)
		for (size_t c = 1; c < width - 1; ++c)
		{
			...
			tmp[index] = static_cast<T>(reciprocalThree * static_cast<CastType>(inputOutput[index - 1] + inputOutput[index] + inputOutput[index + 1]));
		}
	...
	for (size_t c = 0; c < width; ++c)
	{
		...
		inputOutput[firstRowIndex] = static_cast<T>(half * static_cast<CastType>(tmp[firstRowIndex] + tmp[firstRowIndex + width]));

		auto const lastRowIndex = (width - 1) * width + c;
		inputOutput[lastRowIndex] = static_cast<T>(half * static_cast<CastType>(tmp[lastRowIndex] + tmp[lastRowIndex - width]));
	}
	// Middle rows
	for (size_t r = 1; r < height - 1; ++r)
		...
		{
			auto const index = r * width + c;
			inputOutput[index] = static_cast<T>(reciprocalThree * static_cast<CastType>(tmp[index - width] + tmp[index] + tmp[index + width]));
		}
}

/// function to create a gaussian kernel of filterWidth*filterWidth with the specified sigma
template <typename T, bool normalise> std::vector<T> create2DGaussianKernel(size_t filterWidth, T sigma)
{
